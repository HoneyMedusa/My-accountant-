...
export default function App() {
  ...
  const activeUid = user?.uid || GUEST_UID;
  ...
  useEffect(() => {
    const unsubProfile = onSnapshot(doc(db, 'users', activeUid), (docSnap) => {
      ...
      } else if (activeUid === GUEST_UID) {
        setProfile({
          uid: GUEST_UID,
          email: 'guest@example.com',
          businessName: 'My Guest Business',
          createdAt: new Date()
        });
      }
      setLoading(false);
  ...
  const renderView = () => {
    switch (currentView) {
      case 'dashboard':
        return <Dashboard userId={activeUid} />;
      case 'stock':
        return <StockManagement userId={activeUid} />;
      case 'cashups':
        return <CashUps userId={activeUid} />;
      case 'expenses':
        return <Expenses userId={activeUid} />;
      case 'history':
        return <History userId={activeUid} />;
      default:
        return <Dashboard userId={activeUid} />;
    }
  };
